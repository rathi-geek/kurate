...
export async function fetchShareableConversations(): Promise<ShareableConversation[]> {
  ...
  const groupConvos: Array<{ id: string; group_name: string | null; updated_at: string; group_avatar: { file_path: string; bucket_name: string } | null }> = [];
  ...
  for (const row of memberships) {
    ...
    if (!convo) continue;
    if (convo.is_group) {
      groupConvos.push({ id: convo.id, group_name: convo.group_name, updated_at: convo.updated_at, group_avatar: (convo as { group_avatar?: { file_path: string; bucket_name: string } | null }).group_avatar ?? null });
    } else {
  ...
  const groupItems: ShareableConversation[] = groupConvos.map((g) => ({
    ...
    name: g.group_name ?? "",
    type: "group",
    avatar_url: mediaToUrl(g.group_avatar),
  ...
  if (dmConvoIds.length > 0) {
    ...
    dmItems = (otherMembers ?? []).map((m) => {
      ...
      return {
        ...
        name: displayName,
        type: "dm" as const,
        avatar_url: mediaToUrl((rawProfile as { avatar?: { file_path: string; bucket_name: string } | null } | null)?.avatar ?? null),
