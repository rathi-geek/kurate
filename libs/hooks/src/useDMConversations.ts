...
import type { Database, DMConversation } from "@kurate/types";
      ...
      otherUser: {
        ...
        display_name: rawProfile
          ? [rawProfile.first_name, rawProfile.last_name].filter(Boolean).join(" ") ||
            ...
            null
          : null,
        avatar_url: rawProfile?.avatar ? mediaToUrl(supabaseUrl, rawProfile.avatar) : null,
...
export function useDMConversations(
  ...
  const query = useQuery({
    ...
    queryFn: () => fetchDMConversations(supabase, supabaseUrl, userId!),
    enabled: !!userId,
  });

  // Realtime subscription: invalidate conversations list on any new message from others
  useEffect(() => {
    if (!userId) return;
