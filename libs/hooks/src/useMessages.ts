...
import type { Database, DMMessage } from "@kurate/types";
    ...
    sender: {
      ...
      display_name: rawSender
        ? [rawSender.first_name, rawSender.last_name].filter(Boolean).join(" ") ||
          ...
          null
        : null,
      avatar_url: rawSender?.avatar ? mediaToUrl(supabaseUrl, rawSender.avatar) : null,
...
export async function fetchMessages(
  ...
  let query = supabase
    ...
    .select(
      ...
      created_at,
      logged_item_id,
      sender:profiles!messages_sender_id_fkey(id, first_name, last_name, avatar:avatar_id(file_path, bucket_name), handle),
  ...
  return ((data ?? []) as Parameters<typeof mapRow>[1][]).reverse().map((row) => mapRow(supabaseUrl, row));
}
...
export function useMessages(
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => fetchMessages(supabase, supabaseUrl, convoId!, pageParam as string | undefined),
